    ...
    public class EncryptionService : IEncryptionService
    {
        ...
        public virtual string EncryptText(string plainText, string encryptionPrivateKey = "") 
        {
            ...
            if (String.IsNullOrEmpty(encryptionPrivateKey))
                encryptionPrivateKey = AppConstants.EncryptionKey;
        ...
        public virtual byte[] EncryptFile(byte[] file, string encryptionPrivateKey = "")
        {
            if (file.Length == 0)
                return file;
        ...
        public virtual byte[] DecryptFile(byte[] file, string encryptionPrivateKey = "")
        {
            if (file.Length == 0)
                return file;
        ...
        public virtual string Encrypt(string toEncrypt, string key, bool useHashing)
        {
            byte[] keyArray;
        ...
        public virtual string Decrypt(string toDecrypt, string key, bool useHashing)
        {
            byte[] keyArray;
        ...
        private byte[] EncryptFileToMemory(byte[] data, byte[] key, byte[] iv)
        {
            using (var ms = new MemoryStream())
            {
                using (var cs = new CryptoStream(ms, new TripleDESCryptoServiceProvider().CreateEncryptor(key, iv), CryptoStreamMode.Write))
        ...
        private byte[] DecryptFileFromMemory(byte[] data, byte[] key, byte[] iv)
        {
            using (var ms = new MemoryStream(data))
            {
