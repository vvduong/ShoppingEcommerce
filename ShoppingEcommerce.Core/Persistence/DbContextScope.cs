...
namespace ShoppingEcommerce.Core.Persistence
{
    public class DbContextScope : IDisposable
    {
        ...
        public DbContextScope(DbContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel, IDbContextFactory dbContextFactory = null)
        {
            ...
            {
                _nested = false;
                DbContexts = new DbContextCollection(readOnly, isolationLevel, dbContextFactory);
            }
        ...
        public async Task<int> SaveChangesAsync(CancellationToken cancelToken)
        {
            ...
            if (!_nested)
            {
                changes = await CommitInternalAsync(cancelToken).ConfigureAwait(false);
            }
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            foreach (IObjectContextAdapter contextInCurrentScope in DbContexts.InitializedDbContexts.Values)
            {
                var correspondingParentContext = (IObjectContextAdapter) _parentScope.DbContexts.InitializedDbContexts.Values
                ...
                // be refreshed.
                foreach (var toRefresh in entityList)
                {
                    ...
                    if (!correspondingParentContext.ObjectContext.ObjectStateManager.TryGetObjectStateEntry(key,
                        out var stateInParentScope))
                    {
                        continue;
                    }
            ...
            if (_parentScope != null)
            {
                if (_parentScope._disposed)
                {
                    ...
                    var message = @"PROGRAMMING ERROR - When attempting to dispose a DbContextScope, we found that our parent DbContextScope has already been disposed! This means that someone started a parallel flow of execution (e.g. created a TPL task, created a thread or enqueued a work item on the ThreadPool) within the context of a DbContextScope without suppressing the ambient context first. 
                        ...
                        3) Change the code so that the ambient context is suppressed before the parallel task is created. You can do this with IDbContextScopeFactory.SuppressAmbientDbContext() (wrap the parallel task creation code block in this). 
        ...
        internal static DbContextScope GetAmbientScope()
        {
            ...
            if (!(CallContext.LogicalGetData(AmbientDbContextScopeKey) is InstanceIdentifier instanceIdentifier))
            {
