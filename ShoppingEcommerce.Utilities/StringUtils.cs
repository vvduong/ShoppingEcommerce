...
namespace LacViet.ShoppingEcommerce.Utilities
{
    public static class StringUtils
    {
        ...
        public static string ContainsOr(this string value)
        {
            var terms = value.Split(' ');
            value = string.Empty;
            for (var i = 0; i < terms.Length; i++)
            {
                value = string.Concat(value, "\"*" + terms[i] + "*\"");
                if (i < terms.Length - 1)
                    value = string.Concat(value, " OR ");
            }
            return value;
        }
        #endregion
        ...
        #endregion
        #region Get UserName Sharepoint
        public static string GetLoginName(string IdentityName)
        {
            string UserName = string.Empty;
            int index = IdentityName.IndexOf("|") + 1;
            if (index >= 0)
            {
                UserName = IdentityName.Substring(index, (IdentityName.Length - index));
        ...
        #endregion
        #region Multi language get key
        public static string getResourceValueKey(string key)
        {
            return key.Split('.').LastOrDefault();
        }
        #endregion
        public static string testbin()
        {
            return string.Empty;
        }
